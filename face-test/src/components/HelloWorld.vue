  ...
  <div style="width: 100%; position:relative;">
    ...
    <div v-if="detail">
      <x-table>
        <tbody>
        <tr>
          <td>垂直角度（-180~180 顺时针）</td>
          <td>{{detail.location.rotation}}</td>
        </tr>
        <tr>
          <td>人脸旋转（-90左）（90右）</td>
          <td>{{detail.angle.yaw}}</td>
        </tr>
        <tr>
          <td>人脸旋转（-90仰）（90俯）</td>
          <td>{{detail.angle.pitch}}</td>
        </tr>
        <tr>
          <td>水平角度（-180逆时针）（180顺时针）</td>
          <td>{{detail.angle.roll}}</td>
        </tr>
        </tbody>
      </x-table>
      <x-table>
        ...
        <tr>
          <td>脸型</td>
          <td>{{detail.face_shape.type}}</td>
        </tr>
        <tr>
          <td>可信度</td>
          <td>{{detail.face_shape.probability}}</td>
        </tr>
        </tbody>
      </x-table>
      <x-table>
...
<script>
import api from '../../api'
import {AlertModule, XButton, Loading, TransferDomDirective as TransferDom, XTable} from 'vux'
...
export default {
  ...
  methods: {
    ...
    postImg (rawdata) {
      ...
      api
        .post('uploadsImageBase64', data, true)
        .then(response => {
          this.show1 = false
